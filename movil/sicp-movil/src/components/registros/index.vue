<template>
    <ion-content>
        ...
        <ion-list>
            ...
            <ion-item :key="index" @click="optionsSelect(item.codigo,item.finalizado,index)" :class="item.finalizo?'status-finish':'status-pendient'">
                <ion-avatar slot="start">
                    <img v-if="item.finalizado" src="../../assets/success.png">
                    <img v-else src="../../assets/danger.png">
                </ion-avatar>
                <ion-lavel>
                        Codigo de tarjeta familiar {{item.codigo}}, Personas registradas {{item.registros.length}} - {{item.fechaProximaVisita}}
                </ion-lavel>
                <ion-lavel>
                </ion-lavel>
            </ion-item>
            </template>
                    <ion-datetime style="display:none" id="fechas1" :ref="'fechas'" displayFormat="YYYY-MM-DD" month-short-names="Enero, Febrero, Marzo, Abril, Mayo, Junio, Julio, Agosto, Septiembre, Octubre, Noviembre, Diciembre" display-timezone="utc" @ionChange="fecha = $event.target.value" :value="fecha"></ion-datetime>
        </ion-list>
    </ion-content>
</template>
